...
Here, I load in the libraries that are required to run future functions.
...
I collect the two datasets from online websites and save them into RDS objects in case I want to use them in the future (without having to collect from online again).
...
We save the data into RDS objects in our data/ directory.
...
We convert the second dataframe from wide to long format. In the process of doing this, we also clean up some of the strings in variables and reformat the data to be easier and cleaner to read. We save this to a new object.
...
```
We also wrangle and clean up the first dataframe. We separate the date into month/year and record sales per year in the same format that is recorded by the second dataframe. This is also saved to a new object.
...
We create a new dataset that contains information from the previous individual datasets.
```{r merge-data, warning=FALSE} 
## Now, combine the two data frames
df_total <- bind_rows(df_retail, df_department)
```

We create scatterplots of the data from each of the three datasets to better understand what our data looks like and what patterns we see.
